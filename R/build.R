...
POPbuild <- function(survey1,
  ...
  if (is.null(theta.ini)) {
    ...
    deltaC <- 0
    upsilonC <-
      5 # different meaning than in Andy's code, smaller than his
    muS1 <- 13.3
    deltaS1 <- 0.22
    upsilonS1 <-
      10  # different meaning than in Andy's code, smaller than his
    h <- 0.674
    qS1 <- 1
    qS2 <- 1
    qS3 <- 1
  ...
  tUS1 <-
    which(catch[, 1] %in% paa.survey1.female[, 1]) - 1L # C++ indices
...
# TODO: Sean: check that this was an error:
