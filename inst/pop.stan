// Quick exercise - Sean: is the code shorter/more transparent/easier to
// maintain directly in Stan plus can sampling performance be improved? is MPD
// optimization as fast as TMB anways? work in progress... should be easy to
// translate since both C++ like.
// plus is tmbstan giving same result?
...
functions {
vector invlogitSelectivity(int a, Type mu, Type ups){
...
// Data
data {
  int<lower=1> TC; // t = 1, ..., T=74
  int<lower=1> TS1;
  int<lower=1> TS2;
  int<lower=1> TS3;
  int<lower=1> UC;
  int<lower=1> US1;
  int<lower=1> A; // a = 1, ..., A=30

  int<lower=1> G; // number of fleets // TODO: config
  int<lower=1> indC; // index for catch data
  int<lower=1> indS1; // g index for S1
  int<lower=1> indS2; // g index for S2
  int<lower=1> indS3; // g index for S3

  real<lower=0> Ct[TC]; // commercial catch, g=1, dim TC
  real<lower=0> S1t[S1t]; // abundance index survey g=2, dim TS1
  real<lower=0> S2t[S2t]; // abundance index survey g=3, dim TS2
  real<lower=0> S3t[S3t]; // abundance index survey g=4, dim TS3
  
  matrix[A, UC];  // weighted proportion commercial catch females, dim AxUC
  matrix[A, UC];  // weighted proportion commercial catch males, dim AxUC
  int<lower=0> ntC[UC];    // observed total counts catch, dim UC
  matrix[A, US1] patS11; // weighted proportion survey 1 females, dim AxUS1
  matrix[A, US1] patS12; // weighted proportion survey 1 males, dim AxUS1
  int<lower=0> ntS1;   // observed total counts survey S1, dim US1
  
  int<lower=1> tS1[TS1]; // indices of S1t values within 0:(TC-1), dim TS1
  int<lower=1> tS2[TS2]; // indices of S2t values within 0:(TC-1), dim TS2
  int<lower=1> tS3[TS3]; // indices of S3t values within 0:(TC-1), dim TS3
  int<lower=1> tUC[UC]; // indices of patC values within 0:(TC-1), dim UC
  int<lower=1> tUS1[US1]; // indices of patS1 values within 0:(TC-1), dim US1
  
  real<lower=0> wa1[A]; // average weight females, dim A
  real<lower=0> wa2[A]; // average weight males, dim A
  real<lower=0, upper=1> ma[A]; // proportion females mature, dim A
  real<lower=0> kappaS1[TS1]; // known sd of obs error of S1t, dim TS1
  real<lower=0> kappaS2[TS2]; // known sd of obs error of S2t, dim TS2
  real<lower=0> kappaS3[TS3]; // known sd of obs error of S3t, dim TS3
  
  // TODO-Sean lower bounds?
  real muS2; // age full selectivity females, S2
  real deltaS2; // age shift full selectivity males, S2
  real upsilonS2; // log scale selectivity curves, S2
  real muS3; // age full selectivity females, S3
  real deltaS3; // age shift full selectivity males, S3
  real upsilonS3; // log scale selectivity curves, S3
  
  real<lower=0> sigmaR; // sd of proc error of Rt // TODO-workshop: weighting
  
  int<lower=1, upper=2> lkhdpropatage; // 1 = Gaussian , 2 = binomial
  int<lower=0, upper=1> varweight; // 0 = disabled , 1 = inflated var propatage
  int<lower=0, upper=1> enablepriors; // 0 = disabled , 1 = all enabled
}
parameters {
  ...
  PARAMETER(logupsilonC); // log scale selectivity curves, catch // TODO-workshop: logistic?
...
if (enablepriors==1){ // enabled, all values from Table F.4, p. 83
	Type LBR0 = 1.0;
	Type UBR0 = 100000.0;
	nll += neglogdunif(R0, LBR0, UBR0, TC); // TODO: not uniform in end
	Type meanM1 = 0.07;
	Type sdM1 = 0.007;
...
// Nat, t=1, s=1,2
for (int a = 0; a < (A-1); a++){
  // TODO-workshop: add comment explaining ... will not match equation because 0 indexing in C++
...
// TODO-workshop: divide by two because mid year; Pope's approximation?
...
for (int t = 0; t < TS1; t++){
	...
	Type sumuwsN2 = 0.0;
	for (int a = 0; a < A; a++){
    // TODO-workshop: might want option to set, e.g. when survey happens rather than u_ats/2
	...
	if(lkhdpropatage==2){ // binomial lkhd
		for (int t = 0; t < UC; t++){
			...
			for (int a = 0; a < A; a++){
				...
				nll -= dbinom(patC2(a,t)*ntC(t), ntC(t), meanpatC2(a,t), true); // TODO-workshop: overdispersion?
			}
		}
	} else {
